...
async function authenticate(
  ...
  log("debug", "init");
  const isAuthenticationRedirect = new URL(
  ...
  if (!isAuthenticationRedirect) {
    ...
    alert(
      "Thanks for authenticating. This web page will now receive the AuthenticationResponse."
    );
  ...
  function receiveUrl(url: URL) {
    ...
    switch (typeof authenticator.receiveAuthenticationResponse) {
      ...
      default:
        log(
          "warn",
          "authenticator.receiveAuthenticationResponse is not a function!",
          { typeofReceiveAuthenticationResponse }
        );
    }
  }
}
